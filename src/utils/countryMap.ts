/**
 * Comprehensive mapping of IANA timezone identifiers to ISO 3166-1 alpha-2 country codes.
 *
 * Sources:
 *   - IANA zone.tab: https://data.iana.org/time-zones/data/zone.tab
 *   - IANA zone1970.tab: https://data.iana.org/time-zones/data/zone1970.tab
 *   - IANA backward: https://data.iana.org/time-zones/tzdb/backward
 */
...
export function getCountryCode(zone: string): string {
  return timezoneToCountry[zone] ?? ''
}
...
export function getRegion(zone: string): string {
  const firstSegment = zone.split('/')[0]
  return regionDisplayNames[firstSegment] ?? firstSegment
}
